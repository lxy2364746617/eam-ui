<template>
    <div class="app-container">
        <div>
            ...
            <!-- 导入 -->
            <!-- <file-import @handleFileSuccess="handleFileSuccess" :downloadTemplateUrl="'/system/supplier/importTemplate'" ref="fileImport"
...
export default {
    name: "Template",
    dicts: ['em_device_state', 'em_device_level','equipment_large_have','equipment_large_base','equipment_large_switch','equipment_common_sf','equipment_common_yw'],
    ...
    computed: {
        ...
        columns() {
            return [
                ...
                { label: "台数",prop: "sum", span: 8, number: true,formDisabled:true},
                { label: "设备厂家",prop: "equipmentManufacturer", span: 8,  },
                { label: "投运日期",prop: "putTime", span: 8, formType: "date", },
                { label: "电压等级(v)",prop: "vcc", span: 8,  },
                { label: "公称容积流量(m³/min)",prop: "common", span: 8,  },
                { label: "排气压力(Mpa)",prop: "pressure", span: 8,  },
                { label: "风包有无",prop: "wind", span: 8, formType: "select", options: this.dict.type.equipment_common_yw, },
                { label: "风包容积(m³)",prop: "windBag", span: 8,  },
    ...
    methods: {
      ...
      arr.forEach(item=>{
        ...
        getDetails(queryParams) {
            this.loading = true;
            getCompressor(queryParams).then(response => {
