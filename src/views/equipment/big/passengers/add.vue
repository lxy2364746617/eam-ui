...
export default {
    name: "Template",
    dicts: ['em_device_state', 'em_device_level','equipment_large_have','equipment_large_base','equipment_large_switch','equipment_passengers_common','equipment_passengers_oil','equipment_common_yw','equipment_common_sf'],
    ...
    computed: {
        ...
        columns() {
            return [
                ...
                { label:"电压等级(v)", prop:"vcc", span: 8, },
                { label:"功率(KW)", prop:"power", span: 8, },
                { label:"最大坡度(°)", prop:"maxSlope", span: 8, },
                { label:"运行距离(m)", prop:"runDistance", span: 8, },
                { label:"运行速度(m/s)", prop:"runSpeed", span: 8, },    

                { label:"运输能力(人/h)", prop:"runCapacity", span: 8, },
                { label:"驱动轮尾轮直径(mm)", prop:"drivingWheelWidth", span: 8, },
                ...
                { label:"猴车实际使用宽度(m)", prop:"carWidth", span: 8, },
                { label:"断轴保护措施", prop:"breakProtect", span: 8, formType: "select", options: this.dict.type.equipment_common_yw, },//(0-有,1-无.)
                { label:"减速器油温检测", prop:"isHave", span: 8, formType: "select", options: this.dict.type.equipment_passengers_oil, },//0-有,1-没有.
                { label:"共巷情况", prop:"common", span: 8, formType: "select", options: this.dict.type.equipment_passengers_common, },//(皮带/轿车)
                { label:"无人值守(是/否)", prop:"unmanned", span: 8, formType: "select", options: this.dict.type.equipment_common_sf, },//(是/否)
    ...
    methods: {
        ...
        getDetails(queryParams) {
            this.loading = true;
            getPassengers(queryParams).then(response => {
