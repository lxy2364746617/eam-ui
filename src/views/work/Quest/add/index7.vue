<template>
  <Wrapper :title="wrapperTitle"
    ...
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
      <el-tab-pane
        style=""
        v-for="item in this.lineList"
        :key="item.lineCode"
        :label="item.lineCode + '  ' + item.executeNum + '/' + item.total"
        :name="item.lineCode"
      >
        <span class="lineName">路线名称:{{ item.lineName }}</span>
        <!-- 表格 -->
        <el-table
          v-loading="loading"
          :data="equipmentList"
    ...
    if (localStorage.getItem("item")) {
      ...
      getWomLine({ orderCode: this.formData.orderCode }).then((res) => {
        if (res.code == 200) {
          ...
          this.lineList = [
            {
              lineCode: "RX2023100003",
              lineName: "保养路线测试001",
              executeNum: 2,
              total: 8,
            },
            {
              lineCode: "RX-2023110004",
              lineName: "阿三打撒打撒",
              executeNum: 2,
              total: 8,
            },
