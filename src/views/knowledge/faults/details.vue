  ...
  <div class="pageStyle">
    <el-card class="box-card1" shadow="never">
      ...
      <div class="detail_item">
        ...
        <el-table :data="table.list" stripe style="width: 100%">
          <el-table-column
      ...
      <div class="detail_item">
        <div class="subtitle">
          <i class="el-icon-caret-right"></i> 对策补充
          <span v-if="disabled3" class="rightbutton">
            <el-button
              type="text"
              icon="el-icon-edit"
              @click="disabled3 = false"
              >编辑</el-button
            >
          </span>
    ...
    getDetails(row) {
      ...
      getWomInfo(params).then((res) => {
        ...
        this.infoList.forEach((item) => {
          ...
          } else if (item.prop == "faultType") item.value = faultNume[2];
          else if (item.prop == "faultName") item.value = faultNume[1];
          else {
            item.value = value;
          }
        });
    ...
    getClxxInfo() {
      ...
      getWomRepairInfo({ orderCode: this.orderCode }).then((res) => {
        ...
        this.infoList1.forEach((item) => {
          if (
            ["finishFlag", "attachmentFlag", "haltFlag"].includes(item.prop)
          ) {
            let val = res.data[item.prop] === 0 ? "否" : "是";
            item.value = val;
          } else if (item.prop == "faultType") {
            item.value = this.faultInfoDTO[item.prop];
          } else {
