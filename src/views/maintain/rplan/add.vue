      ...
      <el-row :gutter="10" style="padding: 0 40px; margin: 10px auto">
        <el-col :span="12">
          <el-form-item label="检验计划编码" prop="planCode">
            <el-input v-model="form.planCode" disabled /> </el-form-item
        ></el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="检验类型" prop="itemType">
            <el-select v-model="form.itemType" placeholder="请选择检验类型">
              <el-option
                v-for="dict in dict.type.mro_c_item_type"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              ></el-option>
        ...
        <el-col :span="12">
          <el-form-item label="检验周期类别" prop="planCycleType">
            <el-select
              v-model="form.planCycleType"
              placeholder="请选择保养周期类别"
            >
              <el-option
        ...
        <el-col :span="12">
          <el-form-item label="下次执行日期" prop="nextExecuteTime">
            <el-date-picker
              ...
              :disabled="planId != '' && planId ? true : false"
            ></el-date-picker> </el-form-item
        ></el-col>
...
export default {
  ...
  methods: {
    ...
    getDetails(queryParams) {
      this.loading = true;
      getRplan(queryParams)
        .then((response) => {
          let { mroRegularPlanArchivesList, fileResourceList, ...other } =
            response.data;
          this.form = other;
        ...
        this.$modal
          ...
          .then(function () {
            that.lineList = that.lineList.filter((element) => {
              return !that.selectArr.some(
                (item) => item.deviceId === element.deviceId
              );
            });
          })
          .catch(() => {});
