<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['maintain:classes:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="CLASSESList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="序号" align="center" prop="classesCode" />
      <el-table-column label="班次编码" align="center" prop="classesCode" />
      <el-table-column label="班次类型" align="center" prop="classesType" />
      <el-table-column label="班次时间" align="center" prop="classesTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.classesTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除运维计划班次管理编号为"' + IDs + '"的数据项？').then(function() {
