...
import { getPurchaseList, delProject } from "@/api/property/turnover";
...
export default {
  ...
  methods: {
    ...
    // ! 提交
    sub(val, userIds) {
      definitionStart2(val.id, this.radioRow.transferNo, "device_transfer", {
    ...
    handleControls(row, act) {
      if (act === "add") {
        ...
        this.$modal
          .confirm('是否确认删除单号为"' + row.transferNo + '"的数据项？')
          .then(() => {
            // return delParts(ids);
